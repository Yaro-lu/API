...
- NVIDIA RTX 20 系列或更高，显存 8GB 或以上；8GB 仅适合轻量工作流，WAN 14B、LTX-2.3 等大型工作流以客户端详情页标出的要求为准。
...
客户端和运行环境分开发布。两个安装包都不包含模型或用户数据；升级客户端不会重复下载整套环境，也不会把模型和生成结果打进安装包。
...
首次下载运行环境和模型时需要联网。环境与模型准备完成后，本机生成本身可以离线运行，但客户端默认会尝试建立公网 Tunnel；完全离线使用时请停止公网连接或全部后台服务。
...
不会自动上传。默认生成过程在本机完成；客户端启动后会自动尝试建立带鉴权的公网 Tunnel，只有拿到公网 URL 和有效 API Key 的调用方才能提交任务或读取对应结果。不需要公网访问时，可在客户端停止全部后台服务；请勿公开 URL、API Key 或包含它们的截图。

### 登录平台会同步什么？

只有你主动登录时，客户端才会向你填写的服务端同步公网 URL、本机 API Key、设备名称、工作流/模型状态和不含提示词的任务进度，供平台代为调用。生成提示词和生成结果不在该同步数据中。自定义服务端将取得调用本机接口的凭据，只应登录可信服务；不登录不影响完整本地功能。

### 退出平台登录后，旧的调用权限会自动失效吗？

当前版本退出登录会停止后续平台同步，但不会自动更换已经共享给平台的 API Key。如果不再信任该平台，请先退出登录，再到“设置”中重新生成访问密钥；API 重启后旧 Key 将无法继续调用本机服务。
...
灵境造片厂客户端 ── 工作流路由 ── ComfyUI / 本地模型
        │
        └── Cloudflare Tunnel（客户端默认尝试建立，可停止后台服务关闭）
```
...
- 客户端生成的 API Key 与本机管理 Key 使用当前用户 DPAPI 保护，日志不会输出完整 Key；高级配置中的第三方文本模型 Key 目前以明文保存在本机配置文件中，只应在可信电脑上使用且不得随项目归档。
